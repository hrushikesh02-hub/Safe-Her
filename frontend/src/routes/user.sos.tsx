...
function SOSPage() {
  ...
  const stopCamera = useCallback(() => {
    ...
    // Stop MediaRecorder and remove handlers to avoid endless upload loops
  ...
  async function handleMarkSafe() {
    ...
    // 1. Immediately terminate camera/audio capture and recording hardware
    stopCamera();
  ...
  return (
    <div className="max-w-xl mx-auto py-3 sm:py-6 space-y-5 text-center">
      {/* ===================================================================
        ...
        <div className="space-y-5">
          ...
          <div className="py-6 flex justify-center">
            <button
              ...
              disabled={isSending}
              className="size-44 sm:size-52 rounded-full bg-red-600 hover:bg-red-700 text-white flex flex-col items-center justify-center shadow-xl shadow-red-500/30 active:scale-95 transition-all duration-150 border-4 border-red-100 dark:border-red-900 sos-button-pulse cursor-pointer select-none"
              aria-label="Send Emergency SOS Alert"
            >
              <Siren className="size-12 sm:size-14 mb-1.5 animate-pulse" />
              <span className="text-2xl sm:text-3xl font-black tracking-wider">SOS</span>
              <span className="text-[10px] sm:text-[11px] font-medium text-red-100 mt-0.5">Tap for help</span>
            </button>
          </div>
          ...
          <div className="bg-muted/40 rounded-2xl p-4 sm:p-5 border border-border/60 text-xs text-muted-foreground text-left max-w-md mx-auto space-y-2">
            ...
            <ul className="list-disc pl-4 space-y-1 text-[11px] sm:text-xs leading-relaxed">
              ...
              <li>Live camera evidence is securely recorded for safety admin dispatch.</li>
            </ul>
          </div>
        </div>
      )}
      ...
      {state === "counting" && (
        <div className="space-y-6 py-6">
          <div className="bg-red-50 dark:bg-red-950/40 border-2 border-red-500 rounded-3xl p-6 sm:p-8 text-center space-y-4 max-w-md mx-auto shadow-md">
            ...
            <div>
              <h2 className="text-xl font-bold text-red-950 dark:text-red-200">Sending Emergency Alert...</h2>
            ...
            <div className="pt-2 flex flex-col sm:flex-row gap-2.5">
              <Button
                ...
                variant="outline"
                onClick={cancelSOS}
                className="w-full font-bold text-xs h-11 border-red-300 hover:bg-red-100 dark:border-red-800"
              >
      ...
      {state === "active" && (
        <div className="space-y-4 sm:space-y-5">
          <div className="bg-red-50 dark:bg-red-950/40 border-2 border-red-500 rounded-2xl sm:rounded-3xl p-4 sm:p-6 text-center space-y-4 sm:space-y-5 shadow-md">
            {/* Header */}
            <div className="flex items-center justify-between gap-2 border-b border-red-200 dark:border-red-900/60 pb-3">
              ...
              <span className="font-mono text-xs text-red-900 dark:text-red-300 font-bold bg-white dark:bg-card px-2 py-0.5 rounded-md border border-red-200 dark:border-red-900">
                #{currentAlert?._id?.slice(-6) || "SOS"}
              </span>
            </div>

            <div>
              <h2 className="text-xl sm:text-2xl font-black text-red-950 dark:text-red-200">
            ...
            {/* ASSIGNED VOLUNTEER CARD (Prominently displayed) */}
            <div className="bg-white dark:bg-card rounded-xl sm:rounded-2xl p-3.5 sm:p-4 border border-red-200 dark:border-red-900/60 text-left shadow-xs space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                  <ShieldAlert className="size-3.5 text-primary" /> Assigned Responder
                </span>
              ...
              {volunteerName ? (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-muted/30 p-3 rounded-xl border">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {currentAlert?.estimatedEtaMinutes != null && (
                        <div className="text-xs text-indigo-600 dark:text-indigo-400 font-bold flex items-center gap-1 mt-0.5">
                          <Navigation className="size-3" /> ETA: ~{currentAlert.estimatedEtaMinutes} min arrival
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {volunteerPhone && (
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-3.5 py-2 rounded-lg shadow-xs transition-colors shrink-0"
                    >
                ...
                <div className="p-3 bg-muted/30 rounded-xl border flex items-center gap-3">
                  <div className="size-10 rounded-full bg-amber-500/10 border border-amber-500/30 flex items-center justify-center shrink-0">
            ...
            <div className="rounded-xl sm:rounded-2xl border-2 border-red-400 bg-black p-3 text-white space-y-2 overflow-hidden shadow-inner">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold tracking-wider uppercase text-red-400 flex items-center gap-1">
                    <VideoIcon className="size-3" /> Live Evidence Feed ({recordingSeconds}s)
                  </span>
                </div>
              ...
              {/* Video Element */}
              <div className="relative rounded-xl overflow-hidden aspect-video bg-neutral-900 flex items-center justify-center min-h-[180px] sm:min-h-[220px]">
                <video
                  ref={setVideoRef}
                  autoPlay
                  playsInline
                ...
                />
                {!cameraActive && (
                  <div className="flex flex-col items-center justify-center text-xs text-muted-foreground p-5 text-center space-y-2">
                    <Camera className="size-8 text-white/50 animate-pulse" />
              ...
              <div className="flex items-center justify-between text-[10px] text-white/70 px-1 pt-0.5">
                <span>Storing in database for Safety Admins</span>
                {evidenceUploaded ? (
                  <span className="text-emerald-400 font-bold flex items-center gap-1">
                    <CheckCircle2 className="size-3" /> Stored
                  </span>
                ) : (
                  <span className="text-yellow-300 font-medium">Streaming active...</span>
                )}
              </div>
            </div>

            {/* GPS Location Bar */}
            <div className="bg-white dark:bg-card rounded-xl p-2.5 border border-red-200 dark:border-red-900/50 flex items-center justify-between text-xs">
              <span className="text-muted-foreground flex items-center gap-1">
                <MapPin className="size-3.5 text-red-500" /> GPS Location:
              </span>
            ...
            <div className="pt-1">
              <Button
                ...
                className="w-full h-12 text-xs sm:text-sm font-bold border-red-300 hover:bg-emerald-50 hover:border-emerald-400 hover:text-emerald-800 dark:border-red-900 transition-all shadow-xs"
              >
                {isResolving ? (
      ...
      =================================================================== */}
      {state === "resolved" && (
        <div className="bg-card rounded-2xl sm:rounded-3xl border p-6 sm:p-8 text-center space-y-4 max-w-md mx-auto shadow-xs">
          <div className="size-14 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto border border-emerald-200 dark:border-emerald-800">
            <CheckCircle2 className="size-8" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-foreground">Emergency Resolved</h2>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              The emergency incident has been successfully concluded. Camera recordings have stopped and your contacts have been notified of your safety.
            </p>
          </div>
