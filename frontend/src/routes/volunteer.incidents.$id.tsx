    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {/* Incident Resolution Summary (If Resolved) */}
          {alert.resolutionSummary && (
            <div className="rounded-2xl border border-emerald-500/40 bg-emerald-50/50 p-5 shadow-xs">
              <div className="flex items-center gap-2 font-bold text-emerald-800 mb-3 text-sm">
                <CheckCircle2 className="size-4 text-emerald-600" /> Incident Resolution Summary
              </div>
              <div className="grid sm:grid-cols-2 gap-3 text-xs">
                <div>
                  <span className="text-[11px] text-muted-foreground">Incident Type</span>
                  <div className="font-semibold text-foreground">{alert.resolutionSummary.incidentType}</div>
                </div>
                <div>
                  <span className="text-[11px] text-muted-foreground">Priority Level</span>
                  <div className="font-bold text-red-600">
                    {alert.resolutionSummary.priority}
                  </div>
                </div>
                <div>
                  <span className="text-[11px] text-muted-foreground">Dispatch Response Duration</span>
                  <div className="font-semibold text-foreground">{alert.resolutionSummary.assignmentDurationSec} seconds</div>
                </div>
                <div>
                  <span className="text-[11px] text-muted-foreground">Total Incident Duration</span>
              ...
              {alert.resolutionSummary.mainFactors && alert.resolutionSummary.mainFactors.length > 0 && (
                <div className="mt-3 pt-2 border-t border-emerald-200">
                  <span className="text-[11px] text-muted-foreground font-semibold">Primary Factors:</span>
          ...
          <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-xs">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2 text-foreground">
              <Clock className="size-4 text-muted-foreground" /> Incident Timeline
            </h3>
            {timeline.length === 0 ? (
              <p className="text-xs text-muted-foreground">No events recorded yet.</p>
            ) : (
              <div className="space-y-3 relative before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-border">
                {timeline.map((evt, idx) => (
                  <div key={idx} className="relative pl-6 text-xs">
                    <div className="absolute left-1 top-1 size-3 rounded-full bg-indigo-600 border-2 border-background" />
        ...
        {/* Right Col: Victim & Detection Context */}
        <div className="space-y-5">
          {/* User Details Card */}
          <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-xs">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2 text-foreground">
              <UserIcon className="size-4 text-indigo-600" /> Person in Need
            </h3>
            <div className="flex items-center gap-3.5 mb-4 pb-3 border-b border-border/60">
              <UserAvatar
                src={alert.user?.profileImage}
                name={alert.user?.name || "SafeHer Member"}
                role="user"
                size="lg"
                className="border-2 border-indigo-100"
              />
              <div className="min-w-0">
                <div className="font-bold text-sm text-foreground truncate">{alert.user?.name || "SafeHer Member"}</div>
                <div className="text-[11px] text-muted-foreground">{alert.user?.isVerified ? "Verified Member" : "Registered User"}</div>
              </div>
            </div>
            <div className="space-y-2.5 text-xs">
              <div>
                <span className="text-[11px] text-muted-foreground">Emergency Contact Phone</span>
                <div className="font-semibold text-indigo-600 flex items-center gap-1.5 mt-0.5">
                  <Phone className="size-3.5" />
                  <a href={`tel:${alert.user?.phone}`}>{alert.user?.phone || "Not provided"}</a>
                </div>
              </div>
              <div>
                <span className="text-[11px] text-muted-foreground">Created Timestamp</span>
          ...
          <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-xs">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2 text-foreground">
              <Activity className="size-4 text-indigo-600" /> Detection Summary
            </h3>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-border/60">
                <span className="text-muted-foreground">Trigger Source:</span>
                <span className="font-semibold text-foreground">{alert.source?.replace("_", " ")}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-border/60">
                <span className="text-muted-foreground">Priority Level:</span>
                <span className="font-bold text-red-600">{alert.priority || "P1"}</span>
              </div>
              {alert.distressType && (
                <div className="flex justify-between py-1 border-b border-border/60">
                  <span className="text-muted-foreground">Acoustic Feature:</span>
                  <span className="font-semibold capitalize text-foreground">{alert.distressType}</span>
                </div>
              )}
              {alert.movementAnomalyType && (
                <div className="flex justify-between py-1 border-b border-border/60">
                  <span className="text-muted-foreground">Movement Feature:</span>
                  <span className="font-semibold capitalize text-foreground">{alert.movementAnomalyType}</span>
                </div>
              )}
              {alert.routeDeviated && (
                <div className="flex justify-between py-1 border-b border-border/60">
                  <span className="text-muted-foreground">Route Status:</span>
                  <span className="font-semibold text-amber-700">Deviated trajectory</span>
                </div>
              )}
              {alert.detectedKeywords && alert.detectedKeywords.length > 0 && (
                <div className="py-1">
                  <span className="text-muted-foreground block mb-1">Detected Keywords:</span>
                  <div className="flex flex-wrap gap-1">
                    {alert.detectedKeywords.map((k, i) => (
                      <span key={i} className="bg-red-50 text-red-700 border border-red-200 px-1.5 py-0.5 rounded font-semibold text-[10px]">
                        "{k}"
                      </span>
                    ))}
                  </div>
            ...
            {alert.priorityReasons && alert.priorityReasons.length > 0 && (
              <div className="mt-3 pt-2 border-t border-border/60">
                <span className="text-[11px] font-semibold text-muted-foreground block mb-1">Key Factors:</span>
